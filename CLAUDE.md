...
- **CI は sitemill のタグ固定（現在 `v0.7.2`）。sitemill の main の変更は自動では反映されない**（ADR 0006）。手元は `../sitemill` への editable 依存のままなので、ローカルで通っても CI で通らないことがある。エンジンの修正を取り込むときは、sitemill でタグを打ってから `.github/workflows/` 4 本（checks・pipeline・search・weekly）の `ref:` を上げる（手順は `docs/runbook/operations.md` の 5 章）
