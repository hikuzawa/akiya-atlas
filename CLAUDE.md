...
- サイトマップの lastmod は `data/state/lastmod.json`（ページの `<main>` の指紋と、中身が最後に変わった日。sitemill ADR 0025）で決まり、日次の full 実行がビルドのたびに更新してコミットする。**手元の `sitemill build` もこのファイルを書き換えるが、コミットしない**（`git checkout -- data/state/lastmod.json`、無かったなら消す）。6,400 行の同じファイルを日次のコミットと取り合うと、日次のデータの push が競合で止まる。日付で変わる表示を `<main>` に足すときは `data-sitemill-volatile` を付ける（付け忘れは実行レポートの `build.lastmod_changed` が毎日ほぼ全ページになることで分かる）
...
- **CI は sitemill のタグ固定（現在 `v0.7.4`）。sitemill の main の変更は自動では反映されない**（ADR 0006）。手元は `../sitemill` への editable 依存のままなので、ローカルで通っても CI で通らないことがある。エンジンの修正を取り込むときは、sitemill でタグを打ってから `.github/workflows/` 4 本（checks・pipeline・search・weekly）の `ref:` を上げる（手順は `docs/runbook/operations.md` の 5 章）
