# 04. sitemill / pages.py 側への提案（このセッションではコードを触らない）

テンプレートと静的アセットだけで実現できないものを、影響の小さい順に挙げる。いずれも本線セッションが取り込むかを判断する。
テンプレートは「変数があれば使い、無ければ代替表示」の書き方にして、提案の採否に依存しないようにする。

## A. `pages.py` の context 追加（akiya-atlas 側、小さい）
1. **物件行に数値を足す**（`listing_row` と `search_index`）: `price_yen: int | None`、`rent_yen: int | None`、`floor_area_m2: float | None`、
   `land_area_m2`、`layout`、`structure`、`has_detail: bool`（`detail_url` があるか）。
   目的: 価格の中央値・最安の計算、「詳細あり」バッジ、カードの面積・間取り表示。今は `price` が文字列（"350万円"）で計算できない。
2. **集計値を渡す**: 都道府県・市町村・トップに `stats` として `price_median`、`price_min`、`built_min`、`built_max`、`built_median`、
   `crawled_municipalities`（公開中の市町村数）、`subsidy_municipalities`（補助金ありの市町村数）、`band_counts`（価格帯ごとの件数）。
   都道府県ページには `chart_built`（築年の分布）も。
3. **都道府県行に `crawled` と `subsidy_count` を足す**（トップの都道府県カードのミニバー用）。
4. **`recent` 行に補助金フラグ**（トップの「最近確認した物件」カードのバッジ用）。

## B. sitemill.charts（汎用。他サービスでも使う）
1. **`chart_css` の読み込み順**: サービス側 CSS で上書きしやすいよう、`<style>{{ chart_css }}</style>` を先、`style.css` を後に置く
   （テンプレート側で対応可能。sitemill 側では `_LIGHT_TOKENS` を `:root` レベルのトークン参照にすると更に扱いやすい）。
2. **`flow_diagram` の可読性**: 箱 130×64px・文字 11px 相当は小さい。`box_w`/`box_h`/フォントサイズを引数にし、スマホでは
   縦 1 列（`per_row=1`）に切り替えられるようにする。番号は箱の外の丸バッジに。
3. **`choropleth(svg_text, values, classes)` の追加**: 事前に用意した SVG（`id` = 地域コード）に、値に応じたクラスと `<title>`、リンクを
   付けて返す汎用関数。今回はテンプレート（Jinja）で行うが、他サービス（都道府県 × 市町村の集計を持つサイト全般）でも使える。
4. **`mini_bars(bins)`（スパークライン）**: 数字の帯用の小さな分布。今回は Jinja マクロで実装する。

## C. データ・ライセンス
1. 地図データ（国土数値情報 N03、CC BY 4.0）の出典表記を `/data/` の「埋め込み・図解の利用元」に追加する文言をテンプレート側で入れる。
   もし sitemill にライセンス台帳（データ源ごとの判定と出典）を持たせるなら、そこに登録する。
2. sitemill のホワイトリストに **公共データ利用規約（第 1.0 版、PDL1.0）** を追加する提案。国土数値情報のサイト全体の規約が 2026-03-23 から
   PDL1.0 に移行しており、個別データが CC BY 4.0 表記でないものも出てくる。PDL1.0 は政府標準利用規約 2.0 の後継で CC BY 4.0 互換。

## D. 将来（写真）
- `Listing` に `photos: list[{url, license, credit, checked_on}]` を持たせ、ライセンス判定がホワイトリストに一致したものだけ出力する。
  テンプレートには `photo_slot` の置き場所を先に用意しておく（データが無い間は出力しない）。

## E. sitemill の docs に置きたい知見（2026-09-15）

akiya-atlas のデザイン見直しで出た判断のうち、サービスに依らないもの。sitemill の
`docs/` に置けば japan-open-today でも効く。**このセッションは sitemill を触らないので、
置きたい内容としてここに残す。**

### E-1. 表示の解決と転送量の解決は別

`<details>` で畳むのは「長すぎる」への答えで、「重すぎる」への答えではない。HTML は
配信される。`/owners/` は補助制度 2,939 件を畳んだうえで 322KB（gzip）を配っていた。
一覧が長いときは、まず**そのページに全件を置くべきか**を決める。置かなくてよいなら、
畳むのではなく別ページへ送る。

### E-2. ページの重さは圧縮後で測る

繰り返しの多い HTML は 6〜13% まで縮む。生の大きさで判断すると、対応が要らないページに
手を入れ、要るページを見落とす。実測では生 251KB のページが gzip 33KB で問題なし、
生 3,012KB のページが gzip 322KB で要対応だった。`sitemill build` の結果に
**種別ごとの gzip 後の大きさ**を出せると、この判断が毎回できる（提案）。

手元の gzip と本番の brotli（Cloudflare）は 1KB 以内で一致するので、ビルド結果に出せば
配置しなくても転送量を判断できる。なお Lighthouse をローカルで回すときは、`http.server` が
圧縮しないため非圧縮の数字が出る。換算を忘れると、削る必要のない資材を削りにいくことになる。

### E-3. 畳むかどうかは件数ではなく役割で決める

同じ数百件でも、「眺めるもの」（補助制度）は先頭を見せて畳んでよく、「探すもの」
（市町村の索引）は畳むと毎回開く操作が要る。畳むときは見出しに総件数を必ず出す
（畳んだ中身を「無い」と誤読させない）。

### E-4. 信頼ブロックの出典は代表だけでよい

`build/trust.py` の検査は `data-sitemill-trust` の有無しか見ていない。全件を並べると
フッターが本文と同じ大きさになる。**代表を数件＋一覧ページへの導線**が既定の作法として
使えるなら、共通マクロ側に「N 件まで＋残りの件数と導線」の形を用意する余地がある（提案）。

### E-5. ライト固定のサイトでダーク分岐を出さない選択肢

`sitemill.charts.svg.chart_css()` は `prefers-color-scheme: dark` と `:root[data-theme="dark"]` の
2 つの分岐を必ず出力する。ライト固定にしたサイト（akiya-atlas、ADR 0015）では、この宣言が
効かないまま全ページの `<style>` に載る。`<html data-theme="light">` を付ければ空振りするので
実害は無いが、**サイト側が配色を 1 つに決めているなら出力しない**選択肢があるとよい。
`site.toml` の `[site]` か `chart_css(scheme="light")` のような形（提案）。

## F. 生 HTML のキャッシュが、コミット済みの巡回状態より古くなる穴（2026-09-16）

sitemill ADR 0024 の「コミットするものはキャッシュしない」は守れていても、**キャッシュしてよい
もの（生 HTML）が、コミットしたもの（巡回状態）より古くなる**経路が残っている。akiya-atlas では
起きた形跡は無いが、形は japan-open-today の事故と同じ（キャッシュの復元が新しい事実を古い版で
置き換える）。v0.5.2 のコードで確かめた。

### どう起きるか

1. `fetch/crawler.py` は、生 HTML のキャッシュ（`data/raw`）が**あれば**条件付き GET を使い、
   ETag は**コミット済みの** `data/state/crawl.json` から取る
2. サービスの `pipeline.yml` は、状態のコミットを配置より前に行う。一方 `actions/cache` の保存は
   **ジョブが成功したときだけ**
3. 状態をコミットした後に配置で失敗すると、新しい ETag はコミットされ、新しい生 HTML は保存されない
4. 次の実行は 1 つ前の生 HTML を復元し、新しい ETag で 304 を受け、**古い HTML を今の内容として使う**
   （リンクの発見と、`pending_extract` が残っていれば抽出）
5. `commands.py` の抽出は `raw.load_text()` の中身を確かめずに読み、provenance には**状態の新しい**
   `content_hash` を書く。古い HTML から取った値が、正しい抽出に見える

### 直し方の提案

**生 HTML のメタデータには既に `content_hash` が入っている**（`raw.save()` の第 4 引数）。
状態の `content_hash` と突き合わせるだけで塞がる。

- **取得（`crawler.py`）**: キャッシュのメタの `content_hash` が `st.content_hash` と違えば、
  条件付き GET を使わない（`use_conditional = False`）。取り直して保存し直す
- **抽出（`commands.py` の `run_source`）**: 読んだ生 HTML のメタの `content_hash` が
  `st.content_hash` と違えば抽出せず、`raw_mismatch` として数えて次の巡回に回す
  （`missing_cache` と同じ扱い）
- ハッシュは生バイト列の sha256 ではなく、**取得時と同じ `normalize_for_hash` を通したもの**で比べる。
  メタに保存済みの値をそのまま使えば、正規化の規則を 2 か所に持たずに済む

どちらも生 HTML の中身を変えないので、サービス側の変更は要らない。テストは respx で「状態の
ハッシュと違うキャッシュ＋304 を返すサーバー」を作れば再現できる。

japan-open-today のセッションが sitemill を触っているので、そちらで実装してもらう想定（ユーザーが渡す）。

## G. もしもの貼り付けで、次の案件の見出しが前の案件の原文に入る（2026-09-17、v0.6.0 で確認）

> **解決済み（2026-09-17、sitemill `2c5a10c`、main。タグは未発行）。** 同じ実データで確かめ直した。
> 30 件（割れていた Hoan Garden Kushiro が 1 件になった）、他の案件の見出しの混入 0 件。
> 種別が変わったのは下の表の 4 件だけで、どれも空になって除外に落ちた。残り 26 件の種別は変わらない。
> `offers screen` は手元の `../sitemill` で動くので、タグを待たずに使える。

`upgrade-0.5.2-to-0.6.0.md` の「もしもの読み取り」を、実データ（`moshimo-fudosan.txt`、原文の案件見出し
29 件）で確かめた。sitemill は `../sitemill` の main（`v0.6.0-5-gf38532a`、`19cee86` `5db66ac` を含む）。

| 手順書の確認項目 | 結果 |
| --- | --- |
| 案件名が提携状況（未申請など）になっていない。本当の名前が広告主の欄に入っていない | **直っている**（0 件） |
| 「本人 NG」だけの空の記録ができていない。1 案件が 2 つに割れていない | 空は 0 件。**割れが 1 件**（Hoan Garden Kushiro）。ほかに見出しの取りこぼしが 1 件（AI鬼管理）、成果条件の行を案件名と読んだものが 1 件（「公式LINEもしくは申込フォームから…の予約完了: 15,000円」） |
| 原文に次の案件の見出しが混ざらず、種別判定が引きずられていない | **直っていない。31 件中 28 件で混ざっている** |

### 形

もしもの一覧は 1 案件が「見出し → `サイト` → `成果 N円` → 提携状況」の 4 行で始まり、その後に同じ見出しを
もう一度置いてから成果条件・可否の札が続く。**先頭の 4 行が、1 つ前の案件の原文の末尾に付いている。**
記録は 2 回目の見出しから始まるので、名前は正しく読めているが、原文（`raw`）が前にずれている。

```
Emanon|WordPress(ワードプレス)テーマの新規購入
WordPressテーマの購入WordPressプラグインの購入: 10%
審査あり
再訪問 90日
Hoan Home Sapporo|【1件10,000円】札幌の不用品回収・空き家片付けの契約   ← 次の案件
サイト
成果 10000円
未申請
```

### 何が引きずられるか

種別の判定は `raw` の語も見るので、**次の案件の見出しの語で種別が決まる**。手順書が例に挙げた症状が残っている。

| 案件 | 判定された種別 | 原因（`raw` に混ざった次の見出し） |
| --- | --- | --- |
| Emanon（WordPress テーマ） | 片付け | Hoan Home Sapporo「空き家片付け」 |
| 税務調査あんしんメンバーシップ | 査定 | ハックベリーズ不動産「相続不動産の無料査定」 |
| 一般社団法人地域文化振興機構（資格講座） | 買取 | おもちゃ買取ドットJP |
| 総合資格学院（建築士講座） | リフォーム | リフォームガイド マンションリノベ |

今回は点数が申請の目安に届かず、どれも保留か除外に落ちたので、申請の判断は誤っていない。
ただし、成果条件の軽い別業種の案件なら、他社の見出しの語で導線に乗って「申請」に上がりうる。

### 直し方の見当

- 「見出し → `サイト` → `成果` → 提携状況」の 4 行の並びを**次の案件の始まり**として切る
  （今は 2 回目の見出しで切っている）
- 切った 4 行は捨てずに、同じ見出しを持つ次の記録の原文の先頭に付ける（提携状況が読める）
- 回帰テストは、この並びの 2 案件ぶんを `tests/fixtures/affiliate/` に置けば足りる。`raw` に他の案件の見出しが
  含まれないことを確かめる

## H. `parse_year` が「築49年(2026年時点)」を 2026 年と読む（2026-09-17、v0.7.2 で確認）

十日町市（新潟県）の一覧は築年を「築年数 築49年(2026年時点)」と書く。LLM はこれを
`built_year_quote` にそのまま入れ、`parse_year` は**基準年の「2026年」を建築年として返す**。
本番の物件ページに「2026年築」と出ていた（1 件）。

`parse_year` は和暦 → 略号 → 西暦の順に探し、**築年数（`築\d+年`）は西暦が見つからなかったときだけ**
`relative_age` として見る。築年数と基準年が並ぶ書き方では、基準年が先に拾われる。

### 直し方の見当

- 引用に `築\d+年` があり、西暦・和暦が「時点」「現在」「基準」を伴う（`(2026年時点)`・`令和8年4月1日現在`）なら、
  **その年を建築年にしない**。値にするなら「基準年 − 築年数」だが、ADR 0004（推定しない）に沿って
  `relative_age` で値にしないのが素直
- 「1969年(改築2007年)」「2000(H12)年新築 築26年」「大正8年(1919年)築105年」は今と同じ結果にする
  （福島県の実データ。建築年が先に書かれ、築年数は添え書き）
- 回帰テストは上の 4 つと「築49年(2026年時点)」で足りる

## I. 引用の前後を見て値を捨てる手段（2026-09-17、akiya-atlas で先に実装）

三原市（広島県）の一覧は、見出しの無い列に掲載日「令和８年 ６月29日」を並べている。LLM は
「令和８年」を `built_year_quote` に選び、80 件が「2026年築」「2025年築」と出た。プロンプトで
「掲載日から年だけを切り出さない」と 2 通りに書いても、Haiku の答えは変わらなかった。

引用そのもの（`令和８年`）は本文に実在するので照合を通り、パーサは引用しか受け取らないので、
**引用が本文で何の一部だったか**を見る場所がエンジンに無い。akiya-atlas は取り込み（`ingest`）で
生 HTML のキャッシュを読み直し、「本文での出現が月日付きの日付の一部で、どこにも築年の見出しが付いて
いない」なら値にしない形で止めた（`service.built_year_is_a_date`。他の 866 件では 0 件しか外れない）。

- 照合（`verify_quote`）のあとに、**引用の直前・直後の本文を渡す任意のフック**を `QuoteField` に足せると、
  サービスが生 HTML を読み直さずに済む（例: `context_check(quote, before, after) -> note | None`）
- ゼロ幅スペース（U+200B）が日付の間に入っていた。`squash` は空白しか消さないので、照合と文脈の判定で
  食い違いうる。`squash` で Cf 類（U+200B〜U+200D・U+FEFF）も消すかは検討に値する
