...
**全国実測（2026-09-15、並列 8）**。見込み（900 自治体・3,000 件・$15）とほぼ一致した。

| 項目 | 実測 |
|---|---|
| 所要時間 | 2 時間前後（県の処理 119 分。北海道だけ 10 分、他は 1〜3 分） |
| 補助金ページが見つかった自治体 | 全 1,747 のうち **853（49%）** |
| 取り込んだページ | 1,979 |
| 取り込んだ制度 | **2,931 件** |
| LLM の費用 | **$10.28**（1 ページ $0.005 前後） |

県ごとの取得率は公式サイトの作りで 26〜75% とばらつく。異常ではない。
...
**`uv` を止めても、その下の python は生き残る。** 起動役を殺しただけで止まったつもりになると、
裏で巡回と進捗の書き込みが続く（2026-09-15 に実際に起こし、残高が無いまま 22 県を空振りで
巡回した）。子まで落とす。

```powershell
$id = Get-Content logs\subsidies.pid
taskkill /PID $id /T /F          # /T で子プロセスごと落とす
Get-CimInstance Win32_Process -Filter "Name='python.exe'" |
  Where-Object { $_.CommandLine -like '*akiya-atlas*' }   # 残っていないことを確かめる
```

止めたあとは `data/runs/subsidies.json` の `updated_at` が進んでいないことも見る。進んでいれば
まだ動いている。

**県の区切りで進捗を保存する**ので、次の実行は止めた県の頭からやり直す（1 県数分で取り戻せる）。

長く走らせるだけなら、`uv` を挟まず実行ファイルを直に呼ぶほうが確実（起動役と本体が分かれない）。

```powershell
.venv\Scripts\akiya-atlas.exe subsidy-backfill
```
